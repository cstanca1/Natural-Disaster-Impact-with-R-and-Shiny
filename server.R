...
shinyServer(function(input, output) {
    ...
    output$populationImpactByState <- renderPlot({
        data <- dt.agg()
        if(input$populationImpactCategory == 'both') {
            data$Affected <- data$INJURIES + data$FATALITIES
        } else if(input$populationImpactCategory == 'fatalities') {
        ...
        p <- p + geom_map(aes(fill = Affected), map = states_map, colour='blue') + expand_limits(x = states_map$long, y = states_map$lat)
    ...
    output$economicImpactByState <- renderPlot({
        ...
        if(input$economicImpactCategory == 'crops') {
            data$Damages <- data$CROPDMG
        } else {
            data$Damages <- data$PROPDMG
        }
        
        title <- paste("Economic Impact", input$range[1], "-", input$range[2], "(Million US$)")
        p <- ggplot(data, aes(map_id = STATE))
        p <- p + geom_map(aes(fill = Damages), map = states_map, colour='blue') + expand_limits(x = states_map$long, y = states_map$lat)
        p <- p + coord_map() + theme_bw()
        p <- p + labs(x = "Long", y = "Lat", title = title)
        print(p)
    })
    ...
    output$economicImpact <- renderChart({
        ...
        economicImpact$yAxis(axisLabel = "Total Damage (Million US$)", width = 80)
